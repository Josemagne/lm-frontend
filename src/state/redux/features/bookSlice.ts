...
interface InitialBookState {
  ...
  selection: {
    ...
    /**
     * Only true if a BookContainer was clicked
     */
 ...
 */
export const fetchBooksBackend = createAsyncThunk(
  "books/fetchBooksBackend",
  async (): Promise<LM_Book[] | any> => {
 ...
 */
export const fetchBooksFrontend = createAsyncThunk(
  "books/fetchBooksFrontend",
  async (): Promise<LM_Book[] | any> => {
    let error: any = null
    const result = await FAPI.getBooks()
      .then((res) => res)
      .catch((err) => {
        error = err
      })
...
export const booksSelector = createSelector(selectBooks, (books) =>
  Object.values(books)
)

const selectSelectedBook = (state: RootState) =>
  state.books.selection.selectedBook
...
const selectIsSelectingBook = (state: RootState) =>
  state.books.selection.isSelectingBook

export const isSelectingBookSelector = createSelector(
  selectIsSelectingBook,
